  ...
  try {
    ...
    if (issuesResult.items.isEmpty) {
      ...
      for (var i = 0; i < issuesResult.items.length; i++) {
        ...
        if (item.errorRatePercent != null) {
          stdout.writeln('   错误率: ${item.errorRatePercent!.toStringAsFixed(2)}%');
        }
        if (item.deviceRatePercent != null) {
          stdout.writeln('   影响设备率: ${item.deviceRatePercent!.toStringAsFixed(2)}%');
        }
