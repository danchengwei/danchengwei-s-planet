...
8. 用户可以选择创建新房间或输入现有房间号加入房间
9. 点击"创建房间"按钮会自动生成一个唯一的房间号并加入该房间
10. 点击"加入房间"按钮会加入用户输入的房间号对应的房间
11. 应用跳转到 WebRtcActivity，并传递房间号参数
12. WebRtcActivity 连接信令服务器并加入指定房间
13. 当有其他用户加入同一房间时，开始 WebRTC 连接过程
14. 通过信令服务器交换 offer/answer 和 ICE candidates
15. 建立点对点的音视频连接
16. 用户可以通过点击"退出房间"按钮离开当前房间
