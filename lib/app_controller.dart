...
import 'services/anr_analysis_service.dart';
import 'services/aliyun_cli_service.dart';
...
class AppController extends ChangeNotifier {
  AppController() {
    final bounds = calendarInclusiveRangeBounds(calendarDaysInclusive: 90); // 默认查询 90 天（全部数据）
  ...
  /// GetIssues 的可选 `Name`：问题名称模糊搜索（与版本无关）
  String listNameQuery = '';

  /// GetIssues/GetIssue 可选 `PackageName`：工作台会话；**非空时优先于** [config.appPackageName]。
  String listPackageNameQuery = '';

  /// 列表页面的应用版本筛选（AppVersion）- 用于 --name 参数进行模糊搜索
  String listVersionFilter = '';

  /// 列表页面的首现版本筛选（FirstVersion）- 用于 --filter 的 firstVersion 参数
  String listFirstVersionFilter = '';

  /// ANR 时间段分析的版本筛选字段（用于 --filter 的 firstVersion 参数）
  String anrAnalysisVersionFilter = '';
  ...
  /// 设置列表页面的应用版本筛选（AppVersion）
  void setListVersionFilter(String version) {
    listVersionFilter = version.trim();
    pageIndex = 1;
    notifyListeners();
  }

  /// 设置列表页面的首现版本筛选（FirstVersion）
  void setListFirstVersionFilter(String version) {
    listFirstVersionFilter = version.trim();
    pageIndex = 1;
    notifyListeners();
  }

  /// 获取样本的 AppVersion（应用版本）
  /// 通过 get-error 接口获取单个样本信息中的应用版本
  Future<String?> fetchSampleAppVersion({
    ...
    try {
      final error = await cliService.getError(
        bizModule: bizModule,
      ...
      final appVersion = error['AppVersion'] as String?;
      return appVersion?.trim();
    } catch (e) {
      print('[样本查询] 获取 AppVersion 失败: $e');
      return null;
    }
  }

  /// 设置 ANR 分析的版本筛选
  void setAnrAnalysisVersionFilter(String version) {
    anrAnalysisVersionFilter = version.trim();
    notifyListeners();
  }
  ...
  /// 直接设置自定义时间范围（用于页面选择特定日期时）
  void setTimeRange(DateTime startDate, DateTime endDate) {
    rangeStartMs = startDate.millisecondsSinceEpoch;
    rangeEndMs = endDate.millisecondsSinceEpoch;
    pageIndex = 1;
    _listPageSizeForApi = pageSize;
    notifyListeners();
  }
  ...
  /// 与列表「全部 / 最近 / 7 天 / 30 天」芯片一致（自然日、最新日为昨天；**最近 = 1 天，全部 = 90 天**）。
  bool matchesQuickCalendarDays(int calendarDaysInclusive) {
    if (calendarDaysInclusive != 1 && calendarDaysInclusive != 7 && calendarDaysInclusive != 30 && calendarDaysInclusive != 90) {
      return false;
    }
    const tol = 120000;
  ...
  /// 阿里云 CLI 服务（懒加载，确保 config 已加载）
  AliyunCliService? _cliService;

  /// 获取或创建 CLI 服务
  AliyunCliService get cliService {
    if (_cliService == null) {
  ...
  /// ANR 时间段统计分析
  AnrAnalysisService? _anrAnalysisService;
  List<AnrPeriodStatistics> _anrStats = [];
  bool loadingAnrStats = false;
  String? anrStatsError;
  AnrTrendAnalysis? _anrTrendAnalysis;

  List<AnrPeriodStatistics> get anrStatistics =>
      List<AnrPeriodStatistics>.unmodifiable(_anrStats);
  AnrTrendAnalysis? get anrTrendAnalysis => _anrTrendAnalysis;
  ...
  /// 别名方法，用于批量操作栏的清空按钮
  void clearSelectedDigestHashes() {
  ...
  Future<void> createProject(String name, String? templateId) async {
    try {
      _workspace.ensureValidActive();
      ...
      if (templateId == 'xueersi') {
        // 学而思网校预设：从 skills 获取
        config = ToolConfig(
          appKey: '28085188',
          appPackageName: 'com.xueersi.parentsmeeting',
          consoleBaseUrl: 'https://emas.console.aliyun.com',
        );
      } else {
        config = ToolConfig();
      }

      _workspace.projects.add(ProjectEntry(id: id, name: n, config: config));
      _hubSelectedProjectId = id;
      await _persistWorkspace();
      notifyListeners();
    } catch (e, st) {
  ...
  Future<void> refreshIssues({bool resetPageSizeToDefault = false}) async {
    ...
    // 检查平台是否支持选中的 bizModule
    if (!PlatformCapabilities.isSupportedBizModule(config.os, activeBizModule)) {
      issuesError = PlatformCapabilities.getUnsupportedMessage(config.os, activeBizModule);
      lastIssues = null;
      notifyListeners();
      return;
    }
    ...
    try {
      lastIssues = await cliService.getIssues(
        ...
        pageIndex: pageIndex,
        pageSize: _listPageSizeForApi,
        firstVersion: listFirstVersionFilter.isEmpty ? null : listFirstVersionFilter,  // FirstVersion 筛选
        appVersion: listVersionFilter.isEmpty ? null : listVersionFilter,  // AppVersion 筛选
      );
    } catch (e) {
  ...
  Future<void> refreshOverviewDashboard() async {
    ...
    if (config.emasUseMockCrashData && config.bizModule.trim().toLowerCase() == 'crash') {
      ...
      final mock = EmasCrashMockData.mockGetIssues(pageIndex: 1, pageSize: 100);
    ...
    var allCrashes = <IssueListItem>[];
    try {
      for (final biz in _overviewBizOrder) {
        try {
          // 检查平台是否支持该 bizModule，不支持则跳过
          if (!PlatformCapabilities.isSupportedBizModule(config.os, biz)) {
            errs[biz] = PlatformCapabilities.getUnsupportedMessage(config.os, biz);
            continue;
          }

          final pageSz = biz == 'crash' ? 100 : 1;
          final r = await cliService.getIssues(
            bizModule: biz,
            os: config.os.trim(),
            startTimeMs: rangeStartMs,
            endTimeMs: rangeEndMs,
          ...
          if (biz == 'crash') {
            ...
            allCrashes = List<IssueListItem>.of(r.items);
          }
        } catch (e) {
  ...
  /// 初始化 ANR 分析服务（内部调用）
  void _initAnrAnalysisService() {
    if (config.emasUseMockCrashData) {
      _anrAnalysisService = null;  // Mock 模式下不初始化真实服务
      return;
    }
    // 使用阿里云 CLI 服务
    _anrAnalysisService = AnrAnalysisService(
      config: config,
    );
  }
  ...
  /// 获取可用的版本列表
  ///
  /// 通过查询指定业务模块在时间范围内的第一条问题，
  /// 获取其 AffectedVersions 字段作为可用版本列表
  /// 获取指定业务模块和时间范围内的所有版本
  /// 内部通过查询所有问题的 firstVersion 字段来收集版本列表
  ///
  /// 使用场景：版本筛选下拉框、版本统计分析等
  Future<List<String>> fetchAvailableVersions({
    required String bizModule,
    required int startTimeMs,
    required int endTimeMs,
  }) async {
    return _fetchAllVersionsFromIssues(
      bizModule: bizModule,
  ...
  /// 内部方法：从问题列表中提取 FirstVersion（问题首次出现版本）
  /// 通过 get-issues 接口快速获取常见 FirstVersion，去重排序后返回
  /// 为了性能考虑，只拉取前 3 页（共 1500 条问题）来收集版本
  Future<List<String>> _fetchAllVersionsFromIssues({
    required String bizModule,
    required int startTimeMs,
    required int endTimeMs,
  }) async {
    final miss = config.validateEmas();
    if (miss.isNotEmpty) return [];

    try {
      final versionsSet = <String>{};
      final cliService = AliyunCliService(config: config);

      // 使用 get-issues 获取问题列表中的 FirstVersion
      print('[版本查询] 从 get-issues 接口收集 FirstVersion...');
      const int pageSize = 500;
      const int maxPages = 3;  // 只拉前 3 页以保证速度
      int pageIndex = 1;

      for (int i = 0; i < maxPages; i++) {
        final result = await cliService.getIssues(
          bizModule: bizModule,
          os: config.os.trim(),
          startTimeMs: startTimeMs,
          endTimeMs: endTimeMs,
        ...
        // 收集问题的 FirstVersion（问题首次出现版本）
        for (final item in result.items) {
          final version = item.firstVersion?.trim();
          if (version != null && version.isNotEmpty && _isValidVersionFormat(version)) {
            versionsSet.add(version);
          }
        }

        print('[版本查询] 已扫描第 $pageIndex 页，当前 FirstVersion 数: ${versionsSet.length}');

        // 如果这一页为空或已达最后一页，停止拉取
        if (result.items.isEmpty) {
          print('[版本查询] 该页无数据，停止分页');
          break;
        }
        final totalPages = result.pages ?? 1;
        if (pageIndex >= totalPages) {
          print('[版本查询] 已达最后一页（共 $totalPages 页），停止分页');
          break;
        }
      ...
      print('[版本查询] 从 get-issues 前 $maxPages 页收集到的 FirstVersion: ${versionsSet.length} 个');

      // 去重排序后返回（从高到低）
      if (versionsSet.isNotEmpty) {
        final versions = versionsSet.toList();
        versions.sort((a, b) => _compareVersions(b, a));  // 倒序：高到低
        print('[版本查询] 排序后的版本列表: $versions');
        return versions;
      }
  ...
  /// 验证版本格式是否为 xx.xx.xx（三位数字版本）
  static bool _isValidVersionFormat(String version) {
    final regex = RegExp(r'^\d+\.\d+\.\d+$');
    return regex.hasMatch(version);
  }

  /// 版本号比较方法
  /// 支持 "10.21.0"、"10.20"、"11.0.0-beta"、"9980901" 等格式
  /// 返回：a > b 时返回正数，a < b 时返回负数，相等时返回 0
  static int _compareVersions(String aStr, String bStr) {
    // 移除非数字和点号的部分（如 -beta）
    final aCleaned = aStr.replaceAll(RegExp(r'[^0-9.]'), '');
    final bCleaned = bStr.replaceAll(RegExp(r'[^0-9.]'), '');

    final aParts = aCleaned.isEmpty
      ? [0]
      : aCleaned.split('.').map((s) => int.tryParse(s) ?? 0).toList();
    final bParts = bCleaned.isEmpty
      ? [0]
      : bCleaned.split('.').map((s) => int.tryParse(s) ?? 0).toList();
  ...
  /// 获取 ANR 时间段统计数据
  ///
  /// [periodCount]: 要统计的周期数（如 3 = 最近 3 周）
  /// [granularity]: 周期粒度（日/周/月）
  /// [topN]: Top N 统计（默认 10，获取前 10 条）
  Future<void> fetchAnrTimeRangeStatistics({
    int periodCount = 3,
    PeriodGranularity granularity = PeriodGranularity.week,
    int topN = 10,
  }) async {
    final miss = config.validateEmas();
    if (miss.isNotEmpty) {
      anrStatsError = '请先完成配置：${miss.join('、')}';
      _anrStats = [];
      _anrTrendAnalysis = null;
      notifyListeners();
      return;
    }

    if (_anrAnalysisService == null) {
      _initAnrAnalysisService();
    }

    loadingAnrStats = true;
    anrStatsError = null;
    notifyListeners();

    try {
      // 获取所选的业务模块（支持 crash/anr/lag/custom/memory_leak/memory_alloc）
      final bizModule = activeBizModule.trim().isNotEmpty ? activeBizModule : 'anr';

      // 获取版本筛选（使用专门的 anrAnalysisVersionFilter 字段）
      final version = anrAnalysisVersionFilter.trim().isNotEmpty ? anrAnalysisVersionFilter.trim() : null;

      _anrStats = await _anrAnalysisService!.fetchAnrStatistics(
      ...
      // 查询成功，无论是否返回数据都不设置错误
      // 数据为空时由 UI 层显示"暂无数据"提示
      if (_anrStats.isNotEmpty) {
        _anrTrendAnalysis = _anrAnalysisService!.calculateTrend(_anrStats);
      } else {
        _anrTrendAnalysis = null;
      }
    } catch (e) {
      // 仅当 API 报错或参数错误时才设置错误状态
      anrStatsError = '查询异常：${e.toString()}';
      _anrStats = [];
      _anrTrendAnalysis = null;
    } finally {
      loadingAnrStats = false;
      notifyListeners();
    }
  }

  /// 生成模拟 ANR 统计数据（用于演示和测试）
  Future<Map<String, dynamic>?> fetchIssueDetail(
    ...
    return await cliService.getIssue(
      bizModule: activeBizModule,
      os: config.os.trim(),
      digestHash: digestHash,
      startTimeMs: t0,
...
''';
  ...
  Future<String> probeEmasConnection([ToolConfig? draft]) async {
    ...
    try {
      // 使用 CLI 服务
      final cliService = AliyunCliService(config: cfg);
      final r = await cliService.getIssues(
        bizModule: biz,
        os: cfg.os.trim(),
        startTimeMs: rangeStartMs,
