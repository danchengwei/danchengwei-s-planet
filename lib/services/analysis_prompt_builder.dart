...
String analysisDirectiveForClarity(StackClarity clarity, {required bool hasGitlabHits}) {
  switch (clarity.level) {
    ...
    case StackClarityLevel.systemFrameworkDominant:
      return '当前堆栈**以系统或框架/so 库代码为主**，缺少明确业务源。请：\n'
          '1. **原因分析**：聚焦信号类型（如 SIGABRT/SIGSEGV）、崩溃帧所在 .so 库的功能、可能的触发场景（内存不足、资源竞争、调用时序等）。**禁止**编造具体业务文件名。\n'
          '2. **分析部分**：从系统调用链推断崩溃发生时的整体上下文（如渲染/网络/生命周期回调），说明哪些环节可能出问题。\n'
          '3. **如何处理**：给出业务侧可执行的排查清单（如检查内存泄漏、资源释放、并发访问、特定机型/系统版本适配），以及建议的防护手段（try-catch、降级策略、异常兜底等）。代码示例用伪代码或通用模式。';
