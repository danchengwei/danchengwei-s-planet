...
class ProjectHubPage extends StatelessWidget {
  ...
  Future<void> _showCreateProjectMenu(BuildContext context) async {
    final choice = await showDialog<String>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('选择项目类型'),
        content: const Text('选择预设模板或自定义项目名称'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, 'preset'),
            child: const Text('预设模板'),
          ),
          TextButton(
    ...
    if (context.mounted) {
      if (choice == 'preset') {
        await _showPresetTemplates(context);
      } else if (choice == 'custom') {
        await _promptName(
          context,
          title: '新建项目',
          hint: '例如：网校安卓主站',
          onSubmit: (s) => controller.createProject(s, null),
        );
      }
    }
  }

  Future<void> _showPresetTemplates(BuildContext context) async {
    final choice = await showDialog<String>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('选择预设模板'),
        ...
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, 'xueersi'),
            child: const Text('学而思网校'),
          ),
          TextButton(
            onPressed: () => Navigator.pop(ctx),
            child: const Text('取消'),
    ...
    if (choice == 'xueersi' && context.mounted) {
      try {
        await controller.createProject('学而思网校', 'xueersi');
        // 创建完成后自动进入项目，然后显示配置提示
        if (context.mounted) {
          await controller.commitHubSelectionAndEnter();
          // 进入后显示提示，引导用户配置 AccessKey
          if (context.mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('请在"配置"中填入 AccessKey ID 和 Secret'),
          ...
          body: Container(
            decoration: BoxDecoration(
              gradient: LinearGradient(
                ...
                colors: [
                  t.colorScheme.surfaceContainerHighest.withValues(alpha: kOpacityLight),
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Material(
                            ...
                            shape: RoundedRectangleBorder(
                              ...
                              side: BorderSide(color: t.colorScheme.outlineVariant.withValues(alpha: kOpacityMedium)),
                            ),
                            ...
                            child: ListView.separated(
                              ...
                              separatorBuilder: (_, index) => Divider(
                                height: 1,
                                color: t.colorScheme.outlineVariant.withValues(alpha: kOpacityLight),
                              ),
                        ...
                        Align(
                          ...
                          child: OutlinedButton.icon(
                            onPressed: () => _showCreateProjectMenu(context),
